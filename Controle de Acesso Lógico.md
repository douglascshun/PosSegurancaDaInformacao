...
# cartões:
Normalmente são 3 tipos, magnético um tipo que hoje em dia é pouco utilizado, seu processo usa magnetização, o segundo é RFID (aproximação) esse por sua vez não é programável, e utiliza uma antena interna e um chip para transmitir seus dados sem contato físico. Atua numa frequência de 125kHz (kiloHartz) para controle de acesso e 13,25MHz/Mifare para pagamentos e segurança. Os que possuem chip são do tipo Mifare, esses por suas vezes tem memória, onde pode-se por certificados digitais por exemplo. 
