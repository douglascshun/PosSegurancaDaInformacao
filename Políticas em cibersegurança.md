[[Aula 8 Ética, políticas e conformidade em cibersegurança]]

# O que define politicas em cibersegurança?
As políticas são um conjunto formal de regras emitidas por uma organização para garantir que o usuário autorizado a a cessar a tecnologia e os ativos de informação da organização cumpram as regras e diretrizes relacionadas à segurança da informação.

# Politicas em cibersegurança
Uma politica de segurança também é considerada um "documento vivo", o que significa que o documento nunca é concluído, e sim atualizado continuamente.

# Exemplo de políticas:
- Acesso a internet e redes sociais 
- Armazenamento de Dados
- Política de uso aceitável (AUP)
- Updates
- Acesso remoto 
- LGP
- Planos de cibersegurança 
- Politicas de senha
- Permissões e Controle de acesso

Normalmente, o CISO lidera o desenvolvimento das políticas, bem como sua atualização. A participação de executivos de outros departamentos permite criar políticas atualizadas de forma colaborativa.

Uma política de segurança cibern